...
order_data <- function(data, date_format, date_column, date_difference){
  ...
  # print(last.row)
  # data_ordered <- data_ordered[1:last.row, ]
  return(data_ordered)
}
...
recommendation_for_all_items_on_load <- function(items_details_df, product_id = NA, origin_country_id = NA, destination_country_id = NA) {
    ...
    # fields_to_be_recommended <- c('productId', 'quality', "itemQtyUnitId", "toleranceLevel" , "toleranceType", "shipmentMode", "profitCenterId", "strategyAccId", "originationCountryId"
    #                               , "loadingLocationGroupTypeId",  "destinationCountryId", "destinationLocationGroupTypeId", "originationCityId", "destinationCityId")
...
#   tryCatch(all_productid <- sapply(unlist_item_details, function(x) x$productId, simplify=F), error=function(error_message){all_productid <<- NA} )
#   tryCatch(all_origin_country_id <- sapply(unlist_item_details, function(x) x$originationCountryId, simplify=F), error=function(error_message){all_origin_country_id <<- NA} )
#   tryCatch(all_destination_country_id <- sapply(unlist_item_details, function(x) x$destinationCountryId, simplify=F) , error=function(error_message){all_destination_country_id <<- NA} )
...
create_inputs_for_item_recommendation <- function(user_data) {
  ...
  for (i in 1:length(itemDetails_all)) {
    tryCatch(all_productid[[i]] <- sapply(itemDetails_all[[i]], function(x) x$productId, simplify=F), error=function(error_message){all_productid <<- NA} )
    tryCatch(all_origin_country_id[[i]] <- sapply(itemDetails_all[[i]], function(x) x$originationCountryId, simplify=F), error=function(error_message){all_origin_country_id <<- NA} )
    tryCatch(all_destination_country_id[[i]] <- sapply(itemDetails_all[[i]], function(x) x$destinationCountryId, simplify=F), error=function(error_message){all_destination_country_id <<- NA} )

  }
  all_productid <- unlist(all_productid)
  all_origin_country_id <- unlist(all_origin_country_id)
  all_destination_country_id <- unlist(all_destination_country_id)
    ...
    for (type in unique_contract_type) {
      for (cp in unique_cp_profiles) {
        ...
        if (length(item_details) == 0){
          ...
          item_df = get_item_dataFrame(item_details)
          ...
          if (anyNA(item_df)){
            cp_recommendation_list[[as.character(cp)]] <- list("on_load" = NA, "on_change" = NA)
              ...
              if(is.na(unique_prod_ids) & (length(unique_prod_ids) == 1)){
...
post_data_to_mongo <- function(refId, object, body, xTenant, contentType, authToken){
  ...
  # host_port = Sys.getenv(c("EKA_CONNECT_HOST_QA"))
  ...
  if(all((body['userId'] %in% userId_in_data) & (body['sourceObjectId'] %in% sourceObjectId_in_data))){
    ...
    POST(url = url_post_data ,add_headers("X-TenantID" = xTenant, "X-ObjectAction" = 'CREATE', "Content-Type" = contentType,"Authorization" = authToken), body = result)
  }
}

recommendation_for_an_user <- function(contract_data_json, user, date_format, app_id, object_name, authToken, contentType, xTenant) {
...
recommend_general_details <- function(data_for_userId, ObjectId, input_user_id, traderUserId) {
  ...
  if (length(data_for_userId) == 0){
    ...
    mes <- "trader user id is not passed in the body!"
    return(mes)
  } else {
...
recommend_item_details <- function(data_for_userId, contract_fields, ObjectId) {
  ...
  if (length(data_vec) < 4) {
    mes <- "Incomplete input data for recommending item details"
    return(mes)
  } else {
    if (length(data_for_userId) == 0){
    ###############################
...
authenticate_user <- function(url_platform, authToken) {
  ...
  raw.result <- GET(url = authenticate_url, add_headers("Authorization" = authToken))
  return(raw.result)
...
function(req){
    ...
    if (status_code(authenticate_result) == 200) {
        user_id_for_recommendatio <- post_body_r$userId
        user_id_for_recommendation <- as.character(user_id_for_recommendatio)

        if (is.null(user_id_for_recommendation)) {
          ...
          mes <- paste0("recommendation for the user", user_id_for_recommendation, "is being calculated.")
...
function(itemsection, productChange, req){
  ...
  if (status_code(authenticate_result) == 200) {
    ...
    if (itemsection == TRUE){
      ...
      # print("++++++++++++++++++++++++++++++++++++++++Recommendation requested for general details++++++++++++++++++++++++++++++++++++++++++")
      # general_details_recommendation <- recommend_general_details(data_for_userId, ObjectId, user_id, trader_id)
      tryCatch( general_details_recommendation <- recommend_general_details(data_for_userId, ObjectId, user_id, trader_id)
        , error = function(error_message) {item_details_recommendation <<- NULL}  )
