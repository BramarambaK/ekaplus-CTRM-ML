...
def merge_stat_by_user(all_apps_stats, app_specific_stats):
    to_delete = ['recentUserAgents', 'recentOSs', 'popularUserAgents', 'popularOSs', 'user_id', 'name', 'recentApps', 'popularApps']
    for data_ in all_apps_stats:
        app_specific_data = []
        for app, data_app in app_specific_stats.items():
            for data_app_ in data_app:
                if 'user_id' in data_app_:
                    if data_['user_id'] == data_app_['user_id']:
                        tmp = {str(app):data_app_}
                        for i in to_delete:
                            if str(i) in tmp[str(app)]:
                                del tmp[str(app)][str(i)]
                        app_specific_data.append(tmp)
        result = {}
        for d in app_specific_data:
            result.update(d)
        app_specific_activity = [result]
        data_['app_specific_activity'] = app_specific_activity
...
def merge_all_stats(app_specific_stats):
    ...
    for k,v in app_specific_stats.items():
        tmp = {}
        # print(f"The app specific stat computed is : {v}")
        if len(v) > 1:
            for tmp_dict in v:
                if len(tmp_dict) > 0:
                    tmp.update(tmp_dict)
...
def create_and_post_payload(recommendation_list, across_users_for_apps_, headers_post, platform_url):
...
app = Flask(__name__)
@app.route("/train-user-activity", methods = ['POST'])
def train_():
    ...
    total_api_time = 0
    tick_ = time.time()
    headers_post = dict(request.headers)
    ...
    if authentication_response is not None:
        if authentication_response.status_code == 200:
            ...
            payload = create_and_post_payload(merged_stats, across_users_for_apps_, headers_get_data, headers_post[platform_url_header])
            return payload
